...
def create_config_file(cfg, path, search_path, search_depth):
    ...
    if 'TOOLBOX_PATH' not in os.environ.keys():
        ...
        toolbox_path = search_bart_installed_location(
            path=search_path, depth=search_depth)
    else:
...
def rescan_bart(search_path, search_depth):
    ...
    if os.path.exists(cfg_path):
        ...
        shutil.copy(cfg_path, os.path.join(dirname, '{}_{}'.format(
...
if not os.path.exists(cfg_path):
    try:
        create_config_file(
            config, cfg_path, search_path='/usr', search_depth=4)
